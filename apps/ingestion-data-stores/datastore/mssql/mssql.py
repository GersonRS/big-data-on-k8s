...
class MSSQL(object):
    ...
    def insert_rows():
        ...
        dt_credit_card = Requests.api_get_request(
            url=url_get_credit_card, params=params
        )
        dt_subscription = Requests.api_get_request(
            url=url_get_subscription, params=params
        )
        ...
        pd_df_bank.to_sql(
            "bank", mssql_engine, if_exists="append", index=False, chunksize=10
        )
        pd_df_credit_card.to_sql(
            "credit_card", mssql_engine, if_exists="append", index=False, chunksize=10
        )
        pd_df_subscription.to_sql(
            "subscription", mssql_engine, if_exists="append", index=False, chunksize=10
        )
        pd_df_company.to_sql(
            "company", mssql_engine, if_exists="append", index=False, chunksize=10
        )
