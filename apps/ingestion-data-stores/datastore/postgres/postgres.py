...
class Postgres(object):
    ...
    def insert_rows():
        ...
        pd_df_beer.to_sql(
            "beer", postgres_engine, index=False, if_exists="append", chunksize=100
        )
        pd_df_coffee.to_sql(
            "coffee", postgres_engine, if_exists="append", index=False, chunksize=100
        )
        pd_df_food.to_sql(
            "food", postgres_engine, if_exists="append", index=False, chunksize=100
        )
        pd_df_dessert.to_sql(
            "dessert", postgres_engine, if_exists="append", index=False, chunksize=100
        )
